...
async function getAccessToken() {
  ...
  // --- ULTIMATE BULLETPROOF KEY REBUILDER ---
  let rawKey = process.env.GOOGLE_PRIVATE_KEY || "";
  let key = rawKey;
  
  if (rawKey) {
    // 1. Strip absolutely everything except the pure base64 characters
    let b64 = rawKey
      .replace(/-----BEGIN PRIVATE KEY-----/gi, "")
      .replace(/-----END PRIVATE KEY-----/gi, "")
      .replace(/\\n/g, "") // remove literal \n text
      .replace(/[\s\r\n]+/g, "") // remove ALL whitespace, hidden returns, and actual newlines
      .replace(/"/g, ""); // remove accidental quotes
      
    // 2. Rebuild into the perfect PEM format required by Node.js crypto
