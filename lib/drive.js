...
/* ── resolve service-account credentials from env (foolproof order) ──
 * 1) GOOGLE_SERVICE_ACCOUNT_B64  — base64 of the WHOLE key JSON  (recommended:
 *    no newline/quote mangling possible, email+key always from the same object)
 * 2) GOOGLE_SERVICE_ACCOUNT       — the raw key JSON as one string
 * 3) GOOGLE_CLIENT_EMAIL + GOOGLE_PRIVATE_KEY  — legacy two-var form
 */
function loadServiceAccount() {
  ...
  // tolerate keys accidentally wrapped in surrounding quotes
  if (key.startsWith('"') && key.endsWith('"')) key = key.slice(1, -1).replace(/\\n/g, "\n");
  return { email, key, src: "pair" };
}
