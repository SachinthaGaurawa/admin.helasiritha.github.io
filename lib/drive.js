...
async function getAccessToken() {
  ...
  // --- BULLETPROOF KEY REPAIR SYSTEM ---
  let key = process.env.GOOGLE_PRIVATE_KEY || "";
  key = key.replace(/^"|"$/g, ""); // Remove accidental quotes
  key = key.replace(/\\n/g, "\n"); // Fix literal \n
  
  // If Vercel crushed the newlines into spaces, rebuild the shape automatically:
  if (key.includes("-----BEGIN PRIVATE KEY-----") && !key.includes("\n")) {
    const parts = key.match(/(-----BEGIN PRIVATE KEY-----)(.+)(-----END PRIVATE KEY-----)/);
    if (parts) {
      key = parts[1] + "\n" + parts[2].replace(/\s+/g, "").match(/.{1,64}/g).join("\n") + "\n" + parts[3];
    }
  }
  // -------------------------------------
